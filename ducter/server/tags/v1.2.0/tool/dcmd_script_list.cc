...
uint16_t   g_port = 0;
int        g_client_id = 0;
string     g_script_prex;
uint32_t   g_script_num=0;
uint8_t    g_script_type=dcmd_api::SCRIPT_TYPE_TASK_CMD;
string     g_user;
string     g_passwd;
///-1：失败；0：help；1：成功
int parse_arg(int argc, char**argv) {
  CwxGetOpt cmd_option(argc, argv, "H:P:c:s:n:t:u:p:h");
  int option;
  while( (option = cmd_option.next()) != -1) {
    switch (option) {
    case 'h':
      printf("Get script list by script type.\n");
      printf("%s  -H host -P port -c client-id -t script_type .....\n", argv[0]);
      printf("-H: server host\n");
      printf("-P: server port\n");
      printf("-c: client id\n");
      printf("-s: script prex\n");
      printf("-t: script type,1:task-cmd;2:opr_cmd;3:cron_cmd;4:monitor_cmd\n");
      printf("-n: script number, 0 for all\n");
      printf("-u: user name.\n");
    ...
    case 's':
      ...
      g_script_prex = cmd_option.opt_arg();
      break;
    case 'n':
      if (!cmd_option.opt_arg() || (*cmd_option.opt_arg() == '-')) {
      ...
      g_script_num = strtoul(cmd_option.opt_arg(), NULL, 10);
      break;
    case 't':
      if (!cmd_option.opt_arg() || (*cmd_option.opt_arg() == '-')) {
...
int main(int argc ,char** argv) {
  ...
  string query_msg;
  dcmd_api::UiScripts query;

  query.set_client_msg_id(g_client_id);
  query.set_script_type((dcmd_api::UiScriptType)g_script_type);
  query.set_script_prefix(g_script_prex);
  query.set_fetch_num(g_script_num);
  ...
  CwxMsgHead head(0, 0, dcmd_api::MTYPE_UI_SCRIPTS, 0, query_msg.length());
  ...
  if (dcmd_api::DCMD_STATE_SUCCESS != reply.state()) {
    ...
    printf("script list:\n");
    for (int i=0; i<reply.scripts_size(); i++) {
      printf("\t%s\n", reply.scripts(i).c_str());
    }
  }
  return 0;
}
